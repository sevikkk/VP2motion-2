/* $Id: xspi_low_level_example.c,v 1.1.2.1 2008/07/28 09:49:50 svemula Exp $ */
/******************************************************************************
*
...
int XSpi_LowLevelExample(u32 BaseAddress)
{
	 ...
	 */
	Control = XIo_In16(BaseAddress + XSP_CR_OFFSET);
	Control |= (XSP_CR_LOOPBACK_MASK | XSP_CR_MASTER_MODE_MASK);
	XIo_Out16(BaseAddress + XSP_CR_OFFSET, Control);
	 ...
	 */
	while ((XIo_In8(BaseAddress + XSP_SR_OFFSET) &
			XSP_SR_TX_FULL_MASK) == 0) {
		XIo_Out8((BaseAddress) + XSP_DTR_OFFSET,
				Buffer[NumBytesSent++]);
	}
	 ...
	 */
	while (!(XIo_In8(BaseAddress + XSP_SR_OFFSET) & XSP_SR_TX_EMPTY_MASK));
	...
	while ((XIo_In8(BaseAddress + XSP_SR_OFFSET) &
		...
		Buffer[NumBytesRcvd++] = XIo_In8((BaseAddress) +
						XSP_DRR_OFFSET);
	}

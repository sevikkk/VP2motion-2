/* $Id: xspi.c,v 1.1.2.2 2008/07/30 15:48:43 sadanan Exp $ */
/******************************************************************************
*
...
int XSpi_Start(XSpi * InstancePtr)
{
	 ...
	 */
	XSpi_mIntrEnable(InstancePtr, XSP_INTR_DFT_MASK);
	 ...
	 */
	XSpi_mIntrGlobalEnable(InstancePtr);
...
* @internal
*
* The reset is accomplished by setting the Software Reset register.
*
******************************************************************************/
			...
			u8 *RecvBufPtr, unsigned int ByteCount)
{
	 ...
	 */
	GlobalIntrReg = XSpi_mIsIntrGlobalEnabled(InstancePtr);
	 ...
	 */
	XSpi_mSetSlaveSelectReg(InstancePtr,
	...
	while (((StatusReg & XSP_SR_TX_FULL_MASK) == 0) &&
		...
		if (InstancePtr->DataWidth == XSP_DATAWIDTH_BYTE) {
			 ...
			 */
			XIo_Out16(InstancePtr->BaseAddr + XSP_DTR_OFFSET - 1,
				* (u16 *)InstancePtr->SendBufferPtr);
			InstancePtr->SendBufferPtr +=2;
			InstancePtr->RemainingBytes-=2;
		} else if (InstancePtr->DataWidth == XSP_DATAWIDTH_WORD){
			/*
			 * Data Transfer Width is Word (32 bit).
			 */
			XIo_Out32(InstancePtr->BaseAddr + XSP_DTR_OFFSET - 3,
	...
	if (GlobalIntrReg == TRUE) { /* Interrupt Mode of operation */
		 ...
		 */
		XSpi_mIntrEnable(InstancePtr, XSP_INTR_TX_EMPTY_MASK);
		...
		while(ByteCount > 0) {
			...
			while ((StatusReg & XSP_SR_RX_EMPTY_MASK) == 0) {
				if (InstancePtr->DataWidth ==
					...
					Data8 = XIo_In8(InstancePtr->BaseAddr +
							XSP_DRR_OFFSET);
					 ...
					 */
					u16 Data16;
					InstancePtr->Stats.BytesTransferred +=2;
					Data16 = XIo_In16(InstancePtr->BaseAddr+
							XSP_DRR_OFFSET - 1);
					if (InstancePtr->RecvBufferPtr != NULL){
					 ...
					 */
					u32 Data32;
					InstancePtr->Stats.BytesTransferred +=4;
					Data32 = XIo_In32(InstancePtr->
							BaseAddr +
							XSP_DRR_OFFSET - 3);
					if (InstancePtr->RecvBufferPtr != NULL){
...
u32 XSpi_GetSlaveSelect(XSpi * InstancePtr)
{
	...
	SsReg = XSpi_mGetSlaveSelectReg(InstancePtr);

	/*
	 * Return the inverse of the hardware register and mask it so
	 * unimplemented bits do not show up as ones
	 */
...
void XSpi_InterruptHandler(void *InstancePtr)
{
	 ...
	 */
	IntrStatus = XSpi_mIntrGetStatus(SpiPtr);
	XSpi_mIntrClear(SpiPtr, IntrStatus);
	...
	if (IntrStatus & XSP_INTR_MODE_FAULT_MASK) {
		...
		(void) XSpi_mGetStatusReg(SpiPtr);
	...
	if (IntrStatus & XSP_INTR_TX_EMPTY_MASK) {
		 ...
		 */
		ControlReg = XSpi_mGetControlReg(SpiPtr);
		XSpi_mSetControlReg(SpiPtr, ControlReg |
					XSP_CR_TRANS_INHIBIT_MASK);
		 ...
		 * software may not care to receive data).
		 */
		StatusReg = XSpi_mGetStatusReg(SpiPtr);
		...
		while ((StatusReg & XSP_SR_RX_EMPTY_MASK) == 0) {
			...
			if (SpiPtr->DataWidth ==
				...
				TempData = XIo_In8(SpiPtr->BaseAddr +
							XSP_DRR_OFFSET);
				if (SpiPtr->RecvBufferPtr != NULL) {
					*SpiPtr->RecvBufferPtr++ = TempData;
				}
			} else if (SpiPtr->DataWidth ==
						XSP_DATAWIDTH_HALF_WORD) {
				u16 TempData16;
				SpiPtr->Stats.BytesTransferred+=2;
				TempData16 = XIo_In16(SpiPtr->
						BaseAddr +
						XSP_DRR_OFFSET - 1);
				if (SpiPtr->RecvBufferPtr != NULL) {
			...
			} else if (SpiPtr->DataWidth == XSP_DATAWIDTH_WORD) {
				u32 TempData32;
				SpiPtr->Stats.BytesTransferred+=4;
				TempData32 = XIo_In32(SpiPtr->BaseAddr +
						XSP_DRR_OFFSET - 3);
				if (SpiPtr->RecvBufferPtr != NULL) {
		...
		if (SpiPtr->RemainingBytes > 0) {
			...
			while (((StatusReg & XSP_SR_TX_FULL_MASK) == 0) &&
				...
				if (SpiPtr->DataWidth == XSP_DATAWIDTH_BYTE) {
					/*
					 * Data Transfer Width is Byte (8 bit).
					 */
					XIo_Out8(SpiPtr->BaseAddr +
						XSP_DTR_OFFSET,
					 ...
					 * (16 bit).
					 */
					XIo_Out16(SpiPtr->BaseAddr +
							XSP_DTR_OFFSET - 1,
						*(u16 *)SpiPtr->SendBufferPtr);

					SpiPtr->SendBufferPtr +=2;
					SpiPtr->RemainingBytes-=2;
				} else if (SpiPtr->DataWidth ==
						XSP_DATAWIDTH_WORD) {
					/*
					 * Data Transfer Width is Word (32 bit).
					 */
					XIo_Out32(SpiPtr->BaseAddr +
						XSP_DTR_OFFSET - 3,
						*(u32 *)SpiPtr->SendBufferPtr);

					SpiPtr->SendBufferPtr +=4;
					SpiPtr->RemainingBytes-=4;
				}
			 ...
			 * may be connected to slave select
			 */
			XSpi_mSetSlaveSelectReg(SpiPtr,
			 ...
			 */
			XSpi_mIntrDisable(SpiPtr, XSP_INTR_TX_EMPTY_MASK);

...
#define RXBD_SPACE_BYTES XLlDma_mBdRingMemCalc(BD_ALIGNMENT, XLWIP_CONFIG_N_RX_DESC)
#define TXBD_SPACE_BYTES XLlDma_mBdRingMemCalc(BD_ALIGNMENT, XLWIP_CONFIG_N_TX_DESC)
...
process_sent_bds(XLlDma_BdRing *txring)
{
	...
	for (i = 0, txbd = txbdset; i < n_bds; i++) {
		struct pbuf *p = (struct pbuf *)XLlDma_mBdGetId(txbd);
		pbuf_free(p);
 		txbd = XLlDma_mBdRingNext(txring, txbd);
	}
...
lldma_send_handler(void *arg)
{
	...
	XLlDma_BdRing *TxRingPtr = &XLlDma_mGetTxRing(&xlltemacif->lldma);
	struct xtopology_t *xtopologyp = &xtopology[xemac->topology_index];
	XLlTemac *xlltemac = &xlltemacif->lltemac;

	/* Read pending interrupts */
	irq_status = XLlDma_mBdRingGetIrq(TxRingPtr);

	/* Acknowledge pending interrupts */
	XLlDma_mBdRingAckIrq(TxRingPtr, irq_status);
	XIntc_mAckIntr(xtopologyp->intc_baseaddr, 1 << xlltemac->Config.LLDmaTxIntr);
...
{
	XLlDma_mBdWrite((bd), XLLDMA_BD_STSCTRL_USR0_OFFSET,
		(XLlDma_mBdRead((bd), XLLDMA_BD_STSCTRL_USR0_OFFSET)
		 | 1));
}
...
_bd_csum_set(XLlDma_Bd *bd, u16_t tx_csbegin, u16_t tx_csinsert, u16_t tx_csinit)
{
	...
	XLlDma_mBdWrite(bd, XLLDMA_BD_USR1_OFFSET, app1);

	/* insert init value */
	XLlDma_mBdWrite(bd, XLLDMA_BD_USR2_OFFSET, tx_csinit);
}
#endif
...
lldma_sgsend(xlltemacif_s *xlltemacif, struct pbuf *p)
{
	...
	txring = &XLlDma_mGetTxRing(&xlltemacif->lldma);
	...
	for(q = p, txbd = txbdset; q != NULL; q = q->next) {
		/* Send the data from the pbuf to the interface, one pbuf at a
		   ...
		   variable. */
		XLlDma_mBdSetBufAddr(txbd, q->payload);
		if (q->len > max_frame_size)
			XLlDma_mBdSetLength(txbd, max_frame_size);
		else
			XLlDma_mBdSetLength(txbd, q->len);
		XLlDma_mBdSetId(txbd, (void *)q);
		XLlDma_mBdSetStsCtrl(txbd, 0);
	...
	if (n_pbufs == 1) {
		XLlDma_mBdSetStsCtrl(txbdset, XLLDMA_BD_STSCTRL_SOP_MASK 
...
_setup_rx_bds(XLlDma_BdRing *rxring)
{
	...
	n_bds = XLlDma_mBdRingGetFreeCnt(rxring);
	if (n_bds == 0)
		return;
	...
	for (i = 0; i < n_bds; i++) {
		 ...
		 * issue a XLlDma_Bd_mSetLast(rxbd) here.
		 */
		XLlDma_mBdSetBufAddr(rxbd, p->payload);
		XLlDma_mBdSetLength(rxbd, p->len);
		XLlDma_mBdSetStsCtrl(rxbd, XLLDMA_BD_STSCTRL_SOP_MASK | XLLDMA_BD_STSCTRL_EOP_MASK);
		XLlDma_mBdSetId(rxbd, p);
...
lldma_recv_handler(void *arg) 
{
	...
	struct xemac_s *xemac = (struct xemac_s *)(arg);
	xlltemacif_s *xlltemacif = (xlltemacif_s *)(xemac->state);
	XLlDma_BdRing *rxring = &XLlDma_mGetRxRing(&xlltemacif->lldma);
	struct xtopology_t *xtopologyp = &xtopology[xemac->topology_index];
	XLlTemac *xlltemac = &xlltemacif->lltemac;

	XIntc_mAckIntr(xtopologyp->intc_baseaddr, 1 << xlltemac->Config.LLDmaRxIntr);
	XLlDma_mBdRingIntDisable(rxring, XLLDMA_CR_IRQ_ALL_EN_MASK);

	/* Read pending interrupts */
	irq_status = XLlDma_mBdRingGetIrq(rxring);

	/* Acknowledge pending interrupts */
	XLlDma_mBdRingAckIrq(rxring, irq_status);
	...
	if ((irq_status & (XLLDMA_IRQ_DELAY_MASK | XLLDMA_IRQ_COALESCE_MASK))) {
		...
		for (i = 0, rxbd = rxbdset; i < bd_processed; i++) {
			p = (struct pbuf *)XLlDma_mBdGetId(rxbd);
#ifdef XLLTEMACIF_USE_JUMBO_FRAMES_EXPERIMENTAL
			...
			if (pq_enqueue(xlltemacif->recv_q, (void*)p) < 0) {
				...
				//xil_printf("enqed pbuf: %x\n\r", p);
			}

			rxbd = XLlDma_mBdRingNext(rxring, rxbd);
		}
		...
		/* _setup_rx_bds -> use XLlDma_mBdRingGetFreeCnt */
		_setup_rx_bds(rxring);
	}

	XLlDma_mBdRingIntEnable(rxring, XLLDMA_CR_IRQ_ALL_EN_MASK);
}

void *
alloc_bdspace(int n_desc)
{
	int space = XLlDma_mBdRingMemCalc(BD_ALIGNMENT, n_desc);
...
init_sdma(struct xemac_s *xemac)
{
	...
	RxRingPtr = &XLlDma_mGetRxRing(&xlltemacif->lldma);
	TxRingPtr = &XLlDma_mGetTxRing(&xlltemacif->lldma);
